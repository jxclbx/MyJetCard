...
def api_photos(request, username):
	...
	pqs = PendingPhoto.objects.filter(user=request.user) if is_owner else None
	...
	if year_val:
		...
		if pqs is not None:
			pqs = pqs.filter(date__year=year_val)
	...
	if month_val:
		try:
			...
			if pqs is not None:
				pqs = pqs.filter(date__year=y, date__month=m)
		except ValueError:
			pass
	...
	draft_count = 0
	if pqs is not None:
		pending_count = pqs.filter(status="pending").count()
		draft_count = pqs.filter(status="draft").count()
